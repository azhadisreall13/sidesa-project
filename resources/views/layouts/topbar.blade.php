<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    <form action="{{ url()->current() }}" method="GET" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
        <div class="input-group">
            <input type="text" name="search" value="{{ request('search') }}" class="form-control bg-light border-0 small" placeholder="Cari sesuatu..."
            ...
            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                ...
                @foreach (auth()->user()->notifications as $notif)
                    @if (is_null($notif->read_at))
                        <form id="formNotification-{{ $notif->id }}" action="/notification/{{ $notif->id }}/read"
                            method="post">
                            <div class="dropdown-item d-flex align-items-center"
                                style="background-color: rgba(115, 194, 251, 0.1); cursor: pointer;"
                                onclick="document.getElementById('formNotification-{{ $notif->id }}').submit()">
                                @csrf
                                @method('POST')
                                <div class="mr-3">
